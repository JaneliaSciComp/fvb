...
exp_name = 'EXT_DL_shi_Apollo_20131203T095325';
%exp_name = 'Test_shi_Apollo_20131105T101856';
datetime = exp_name(end-14:end);

exp_directory{1} = sprintf('/Volumes/flyolympiad/Olympiad_Screen/box/reiser_protocol_test_2013/ColorPreference/6_03/%s/Output/01_6.03_34',exp_name);
exp_directory{2} = sprintf('/Volumes/flyolympiad/Olympiad_Screen/box/reiser_protocol_test_2013/ColorPreference/6_03/%s/Output/02_6.03_34',exp_name);
exp_directory{3} = sprintf('/Volumes/flyolympiad/Olympiad_Screen/box/reiser_protocol_test_2013/ColorPreference/6_03/%s/Output/03_6.03_34',exp_name);


out_dir = sprintf('/Volumes/flyolympiad/Olympiad_Screen/box/reiser_protocol_test_2013/ColorPreference/6_03/%s/Output/',exp_name);
...
dir1_starts = [375 1375 2375 3375 4375 5375]; 
dir2_starts = [875 1875 2875 3875 4875 5875];
...
scatter_x = repmat([1,2,3,4,5,6],1,2);
...
for phase = 1:3,
    ...
    for seqnum = seqs
        ...
        load([sprintf('%02d_6.03_seq%d_analysis_info.mat', phase, seqnum)]);
        ...
        for tube_ind = 1:numtubes
            ...
            for j = 1:(length(dir1_starts)),
                ...
                f_against_stim(seqnum,tubes(tube_ind),j,1:2) = [median(f_left(tubes(tube_ind), (dir1_starts(j)+avg_interval(1)):(dir1_starts(j)+avg_interval(2)) )), ...
                ...
                mean_f_a_stim(tubes(tube_ind),j) = nanmean(f_against_stim(seqnum,tubes(tube_ind),j,1:2));
            end
    ...
    scatter(scatter_x,f_against_stim(seqnum,tubes(tube_ind),:),'*k')
    hold on
    plot([0,6], ones(1,2)*0.5,'--r')
    hold on
    plot(x,mean_f_a_stim(tubes(tube_ind),:))
    ylim([0 1]);
    xlim([0 6])
...
for phase = 1:3,
    ...
    for seqnum = seqs
        for j = 1:(length(dir1_starts))
            temp(1:2) = nanmean(f_against_stim(seqnum,tubes,j,1:2));
...
for phase = 1:3,
    ...
    for seqnum = seqs
        for j = 1:(length(dir1_starts))
            temp(1:2) = nanmean(f_w_stim(seqnum,tubes,j,1:2));
            all_tubes_mean(seqnum,j) = nanmean(temp(1:2));
            all_tubes_std(seqnum,j) = std(temp(1:2));
        end
    end
