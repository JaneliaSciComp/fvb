...
ProgramOptionsParser.addParamValue('PasswordLengthMax', 16, @isnumeric);
ProgramOptionsParser.addParamValue('WindowName', 'Login', @ischar);
ProgramOptionsParser.addParamValue('ImagePath', [], @(x) ischar(x) && exist(x, 'file'));
...
if ~isempty(ProgramOptions.ImagePath)
    [imageData, imageMap] = imread(ProgramOptions.ImagePath);
    PositionGUI = PositionGUI + [0 0 0 size(imageData, 1) + 10];
end
...
BackgroundColor = 'white';  %get(0,'DefaultUicontrolBackgroundcolor');
...
if ~isempty(ProgramOptions.ImagePath)
    imageLeft = floor((PositionGUI(3) - size(imageData, 2)) / 2);
    if ProgramOptions.enterUserName
        imageBottom = 103 + OffsetBottom;
    else
        imageBottom = 83 + OffsetBottom;
    end
    handles.image_axes = axes('Parent', handles.figure1, ...
        'Units','Pixels',...
        'Position', [imageLeft imageBottom size(imageData, 2) size(imageData, 1)]);
    handles.image_handle = image(size(imageData, 2), size(imageData, 1), imageData);
    if ~isempty(imageMap)
        colormap(handles.image_axes, imageMap);
    end
    axis off
    axis image
end
...
handles.pushbutton_OK = uicontrol('Parent',handles.figure1, ...
    ...
    'Style','Pushbutton', ...
    'Units','Pixels',...
    'Position',[PositionLeft+BoxWidth-50 10 50 23], ...
    'FontSize',10, ...
    'String','OK',...
...
handles.pushbutton_Cancel = uicontrol('Parent',handles.figure1, ...
    ...
    'Style','Pushbutton', ...
    'Units','Pixels',...
    'Position',[PositionLeft+BoxWidth+-50-5-50 10 50 23], ...
    'FontSize',10, ...
    'String','Cancel',...
    'HorizontalAlignment', 'Center');
...
if ProgramOptions.enterUserName && isempty(ProgramOptions.DefaultUserName)
