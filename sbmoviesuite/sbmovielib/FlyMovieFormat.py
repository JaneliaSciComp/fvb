...
this file contains classes for dealing with fmf and sbfmf movies


I've made (comparatively) few changes to this file since I took it from
Mtrax (not sure which version); most of the code is theirs

summary of changes:

-- bug fix in FlyMovie.get_frame() (end) and FlyMovie.seek(); it's a 64-bit 
    issue, see comments in code
-- in init_sbfmf, stored more info from header so we can access it later;
    eg, difference_mode was discarded, making it impossible to rewrite the movie
...
class FlyMovie:
	...
	#try:
        ...
        self.version = self.file.read(nbytesver)
        
        # read header parameters
        format = '<4IQ'
        nr,nc,self.n_frames,self.difference_mode,self.indexloc = \
    ...
    def _read_next_frame(self):
        if self.issbfmf:
            ...
            # useful for computing diagnostics
            self.sbfmf_npixels = npixels
...
##            raise NotImplementedError("Reading not implemented for %s format"%(self.format,))
    ...
    def get_frame(self,frame_number):
        ...
        # bug fix: seek_to is retrieved as a numpy.uint64 if the
        #   movie was written on a 64-bit machine; if the value is
        #   greater than 2 ** 31, within the file.seek() context ONLY,
        #   an overflow error will be triggered; solution seems to be:
        #   convert to Python data type first (in this case, it'll be
        #   a long), then all is well (even 32-bit Python deals with
        #   file.seek(long) correctly)
        seek_to = int(seek_to)
    ...
    def seek(self,frame_number):
        ...
        if self.issbfmf:
            ...
            seek_to = self.chunk_start+self.bytes_per_chunk*frame_number
        # see bug fix comment, end of "get_frame()" method above!
        seek_to = int(seek_to)
        self.file.seek(seek_to)
        self.next_frame = None
