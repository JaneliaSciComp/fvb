...
all_vel_data = nan(6, 4, half_num_conditions);
all_dir_data = nan(6, 4, half_num_conditions);

motion_resp = nan(6, half_num_conditions);
dir_resp = nan(6, half_num_conditions);
...
for tube_num = 1:6
    ...
    if tfTubeHasValidData  
        ...
        all_vel_data(tube_num, :, :) = ...
            [pos_vel_data(tube_num, 1:half_num_conditions); ...
             pos_vel_data(tube_num, num_conditions:-1:half_num_conditions+1); ...
            -neg_vel_data(tube_num, 1:half_num_conditions); ...
            -neg_vel_data(tube_num, num_conditions:-1:half_num_conditions+1)];
...
for tube_num = 1:6,
    ...
    if tfTubeHasValidData
        motion_resp_this_tube = motion_resp(tube_num,:) ;
        motion_resp_sd_this_tube = reshape(std(all_vel_data(tube_num, :, :), 0, 2) , ...
                                           size(motion_resp_this_tube) ) ;
        errorbar(motion_resp_this_tube, ...
                 motion_resp_sd_this_tube, ...
                 'k.-', 'MarkerSize', 15) ;
    end
    ...
    box off
    text(half_num_conditions, 1.15*y_lim_vel, ... 
...
text(half_num_conditions, -0.8*y_lim_vel, ...
...
% now save figure 
if (save_plots)
    save2pdf([analysis_detail.exp_analysis_output_path filesep protocol_folder_name '_' sequence '_LinMotion_median_x_velocity_and_average.pdf']);
end
...
for tube_num = 1:6
    ...
    subplot(6, 3, ((tube_num)*3))  % this plots the averaged response to reach speed in column 3
    if tfTubeHasValidData,
        dir_resp_this_tube = dir_resp(tube_num, :) ;
        dir_resp_sd_this_rube = reshape(std(all_dir_data(tube_num, :, :), 0, 2), ...
                                        size(dir_resp_this_tube) ) ;
        errorbar(dir_resp_this_tube, ...
                 dir_resp_sd_this_rube, ...
                 'k.-', ...
                 'MarkerSize', 15)
    end

    set(gca, 'Xtick', (1:half_num_conditions))
    axis([0.75 half_num_conditions+0.25 -0.1*y_lim_DI 0.8*y_lim_DI]);

    box off
    
    text(half_num_conditions, 0.8*1.15*y_lim_DI, ...
         [exp_detail.tube_info(tube_num).Genotype ' / ' exp_detail.tube_info(tube_num).Gender], ...
         'HorizontalAlignment', 'right', ...
         'Interpreter', 'none', ...
         'FontSize', 7)
...
text(half_num_conditions, -0.8*y_lim_DI, ...
     ['DateTime: ' exp_detail.date_time], ...
      'FontSize', 7, ...
...
% now save figure 
if (save_plots)
    save2pdf([analysis_detail.exp_analysis_output_path filesep protocol_folder_name '_' sequence '_LinMotion_direction_index_and_average.pdf']);
end
