function prot_31_41_comparison_plot(exp_directory, out_directory, save_plots, temp_folder, temp, exp_detail, protocol)
...
%% create comparison summary plot with medial velocity
del_t = 1/25;
...
if strcmp(protocol, '3.1') || strcmp(protocol, '4.1')
    X_label = {'G = 25', 'G = 120','UV = 36', 'UV = 200'}; %#ok<NASGU>
    seq4_X_label = {'GL', 'GH','UL', 'UH'};
    seq5_X_label = [0 12 24 36 48 60 72 84];
else
    X_label = {'G = 50', 'G = 255','UV = 6', 'UV = 15'};%#ok<NASGU> %protocol 3.0
    seq4_X_label = {'GL', 'GH','UL', 'UH'};
...
if (save_plots)
    save2pdf([out_directory filesep 'med_vel_comparison_summary.pdf']);
    close(99)
end

%% create comparison summary plot with direction index
del_t = 1/25;
min_num_flies = 2; %10; change here for min number of flies
y_lim_vel = 30; % limit in mm/s on the y axes
y_lim_disp = 180; % length of displacement in mm
...
if strcmp(protocol, '3.1') || strcmp(protocol, '4.1')
    ...
    X_label = {'G = 50', 'G = 255','UV = 6', 'UV = 15'};%#ok<NASGU> %mod from 2.8
    seq4_X_label = {'GL', 'GH','UL', 'UH'};
    seq5_X_label = [2:2:10 15 20 25];%mod from 2.8
end
thin_lw = 0.1; % linewidth for thin lines
...
figure(100)
set(100, 'Position', [100 100 1000 750]);
seq2_t = (1:750)*del_t; % used to read as below, but empty tube 1 will crash this...
...
for tube_num = 1:6
   ...
   if (exp_detail.tube_info(tube_num).n >= min_num_flies) && ...% only generate a plot if flies are in the tube
    ...
    % plot the seq 3 DI motion tuning curve
    subplot(6,4,(tube_num-1)*4 + 2) 
    if (exp_detail.tube_info(tube_num).n >= min_num_flies) && ...% only generate a plot if flies are in the tube
        (length(An(1).res(tube_num).seq3.mean_motion_resp) > 1)  % and if data exists           
        ...
        for k = 1:length(temp)
            plot([0 10], [0 0], thin_ls, 'LineWidth', thin_lw) % 200 just a large number
            hold on
            errorbar(An(k).res(tube_num).seq3.mean_dir_index, An(1).res(tube_num).seq3.std_dir_index, 'k.-', 'MarkerSize', 14, 'Color', temp_color(temp(k)), 'LineWidth', 1)
            text(2, 21 - (k-1)*3.1, [texlabel('Delta') num2str(An(k).res(tube_num).seq3.dir_index_diff, '%0.1f') '; mvf:' ...
              num2str(An(k).res(tube_num).seq3.mov_frac, '%0.2f') ], ...
              'HorizontalAlignment', 'Left', 'Color', temp_color(temp(k)), 'FontSize', 7) 
        end
    end
    ...
    set(gca, 'Xtick', 1:6)
    axis([0.75 6.25 -0.1 0.8]); box off
     
    if tube_num == 1
        text(3.5, 1.04, 'DI, visual motion', 'HorizontalAlignment', 'Center') % put the title higher
        ylabel('DI')
    end
    ...
    if (exp_detail.tube_info(tube_num).n >= min_num_flies) && ...% only generate a plot if flies are in the tube
        ...
        for k = 1:length(temp)
            for tr = 1:4
                plot_range = 375*(tr-1) + (1:375);
                time_range = ((375 + plot_gap)*(tr-1) + (1:375))*del_t;
                plot(time_range, An(k).res(tube_num).seq4.mean_cum_dir_index(plot_range)', 'Color', temp_color(temp(k)), 'LineWidth', data_lw)
                plot(time_range(1)*[1 1], [0 0.85*y_lim_disp]',  thin_ls, 'LineWidth', thin_lw)            
            end
            
            text(0.15*time_range_end, 1.06*y_lim_disp - (k-1)*26, ['PS: ' num2str(An(k).res(tube_num).seq4.cum_dir_index_max, '%-0.2f;') ...
                '  mvf:' num2str(An(k).res(tube_num).seq4.mov_frac, '%0.2f')], ...
              'HorizontalAlignment', 'Left', 'Color', temp_color(temp(k)), 'FontSize', 7)     
        end
    ...
    axis([0 time_range_end -2 8]); box off
    
    set(gca, 'Xtick', 7.5+[0 15+time_gap 30+time_gap*2 45+time_gap*3], 'Ytick', [0 8], 'Xticklabel', seq4_X_label);
    if tube_num == 1
        text(time_range_end/2,10.4,'cum. DI, phototaxis',  'HorizontalAlignment', 'center') 
        ylabel('cum. DI')
    end
    if tube_num == 6
        xlabel('time (s)')
    ...
    if (exp_detail.tube_info(tube_num).n >= min_num_flies) && ...% only generate a plot if flies are in the tube
        ...
        for k = 1:length(temp)
            errorbar(An(k).res(tube_num).seq5.cum_dir_index_peak, An(k).res(tube_num).seq5.cum_dir_index_peak_SE,... 
            'k.-', 'MarkerSize', 14, 'Color', temp_color(temp(k)), 'LineWidth', 1) % seems silly to have 'k.-' but it helps!
         
            text(3.5, 1.0*y_lim_disp - (k-1)*40, ['ZC: ' num2str(seq5_X_label(An(k).res(tube_num).seq5.UVG_cross_dir_index)) ...                
                texlabel('; Delta') 'Pref: ' num2str(An(k).res(tube_num).seq5.UVG_pref_diff_dir_index, '%0.2f;') ...
                ' mvf: ' num2str(An(k).res(tube_num).seq5.mov_frac, '%0.2f') ], ...
                'HorizontalAlignment', 'Left', 'Color', temp_color(temp(k)), 'FontSize', 7)     
        end
    end
    ...
    set(gca, 'Xtick', (1:8), 'Ytick', [-8 0 8], 'Xticklabel', seq5_X_label);
    
    if tube_num == 1
        text(4.5,11,'cum. DI, color preference',  'HorizontalAlignment', 'center')  
        ylabel('cum. DI')
    end
...
if (save_plots)
    save2pdf([out_directory filesep 'DI_comparison_summary.pdf']);
    close(100)
end