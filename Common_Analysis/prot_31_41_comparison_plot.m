   ...
   if (exp_detail.tube_info(tube_num).n >= min_num_flies) && ...% only generate a plot if flies are in the tube
    ...
    if (exp_detail.tube_info(tube_num).n >= min_num_flies) && ...% only generate a plot if flies are in the tube
        ...
        for k = 1:length(temp)
            ...
            text(2, 0.84 - (k-1)*0.124, [texlabel('Delta') num2str(An(k).res(tube_num).seq3.dir_index_diff, '%0.1f') '; mvf:' ...
              num2str(An(k).res(tube_num).seq3.mov_frac, '%0.2f') ], ...
              'HorizontalAlignment', 'Left', 'Color', temp_color(temp(k)), 'FontSize', 7) 
        end
    end
    ...
    if (exp_detail.tube_info(tube_num).n >= min_num_flies) && ...% only generate a plot if flies are in the tube
        ...
        for k = 1:length(temp)
            ...
            text(0.15*time_range_end, 8 - (k-1)*0.2222, ['PS: ' num2str(An(k).res(tube_num).seq4.cum_dir_index_max, '%-0.2f;') ...
                '  mvf:' num2str(An(k).res(tube_num).seq4.mov_frac, '%0.2f')], ...
              'HorizontalAlignment', 'Left', 'Color', temp_color(temp(k)), 'FontSize', 7)     
        end
    ...
    if (exp_detail.tube_info(tube_num).n >= min_num_flies) && ...% only generate a plot if flies are in the tube
        ...
        for k = 1:length(temp)
            ...
            text(3.5, 8 - (k-1)*0.2222, ['ZC: ' num2str(seq5_X_label(An(k).res(tube_num).seq5.UVG_cross_dir_index)) ...                
