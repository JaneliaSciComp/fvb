function exp_detail = prot_529_analysis(exp_directory, save_plots, folder_path, temp, phase, AD, protocol, analysis_version)
...
if phase == 2
    ...
    %% now seq9, green constant, median_velocity_x and preference index
    sequence = 'seq9';
    min_num_flies = 2; % minimum number of flies in tubes
    del_t = 1/25; %inverse frames per second
    ...
    try
        load([analysis_detail.seq(9).path '_analysis_info.mat'])
    catch
        error(['No analysis_info in directory ' pwd ' bad name or analysis not run'])
    end
